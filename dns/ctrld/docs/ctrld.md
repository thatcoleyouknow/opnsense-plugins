...
- **HA/CARP config sync covers config.xml, not the ctrld binary itself.**
  This plugin registers a `ctrld_xmlrpc_sync()` hook (System > High
  Availability > Settings) so `Services > ctrld`'s configuration syncs to
  a CARP backup node the same way core services' config does. That does
  *not* install the `ctrld` binary on the backup node -- it's still your
  responsibility to have it present there too (same posture as the
  primary; see the FreeBSD-port note above). Each node's own
  `patch_listener_ips.php` resolves its own live interface addresses
  independently after sync, which is the correct behavior -- a backup
  node's WireGuard/DHCP-assigned interfaces have their own addresses, not
  the primary's.
